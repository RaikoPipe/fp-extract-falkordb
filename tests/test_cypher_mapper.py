...
def test_model_to_cypher_merge_basic():
    r = Resource(name="AKL-01", name_has_index=True, description="AS/RS in zone A", resource_type="AS/RS", capacity=500)
...
def test_model_to_cypher_merge_omits_none_fields():
    r = Resource(name="M-100", name_has_index=True, description="A machine", resource_type="machine")
    query, params = model_to_cypher_merge(r, "Resource")
...
def test_extraction_to_cypher_nodes_and_relationships():
    graph = FactoryPlanningGraph(
        resources=[
            Resource(name="M-100", name_has_index=True, description="Machine 100", resource_type="machine", shift_model="Day-Shift"),
        ],
        control_strategies=[
            ControlStrategy(
                name="FIFO-Dispatch",
...
def test_model_to_cypher_merge_overwrite_mode_unchanged():
    ...
    r = Resource(name="AKL-01", name_has_index=True, description="AS/RS", resource_type="AS/RS", capacity=500)
    query, params = model_to_cypher_merge(r, "Resource")
    assert "MERGE (n:Resource {name: $name})" in query
    assert "SET" in query
...
def test_extraction_to_cypher_default_mode_is_overwrite():
    graph = FactoryPlanningGraph(
        resources=[Resource(name="M-1", name_has_index=True, description="M1", resource_type="machine")],
    )
...
def test_model_to_cypher_fetch_returns_read_only_match():
    r = Resource(name="AKL-01", name_has_index=True, description="AS/RS", resource_type="AS/RS")
